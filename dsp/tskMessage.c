...
//loadMat function def
/*
void loadMat(ControlMsg **msg)
{
	int j,k;
	ControlMsg *temp= malloc(sizeof(ControlMsg));
	for(j=0;j<SIZE;j++)
		{
			for(k=0;k<SIZE;k++)
			{
...
void matMult(ControlMsg **msg)
{
	...
	for (i = 0;i < SIZE; i++)
	{
		for (j = 0; j < SIZE; j++)
		{
			temp->prod[i][j]=0;
			for(k=0;k<SIZE;k++)
				temp->prod[i][j] = temp->prod[i][j]+ temp->mat1[i][k] * temp->mat2[k][j];
...
Int TSKMESSAGE_execute(TSKMESSAGE_TransferInfo* info)
{
	...
	//msg->response=msg->val[0]+msg->val[1];
	/*for(j=0;j<row;j++)
		{
			for(k=0;k<col;k++)
			{
			msg->response[j][k]= msg->val[j][k]*4;	
			//SYSTEM_1Print(" %d\n",  msg->val[i][j]);	
			}
		}     */
		...
		for (j = 0;j < SIZE; j++)
	{
		for (k = 0; k < SIZE; k++)
		{
			msg->prod[j][k]=0;
			for(l=0;l<SIZE;l++)
				msg->prod[j][k] = msg->prod[j][k]+ msg->mat1[j][l] * msg->mat2[l][k];
